...
- **可配置的 discuss 严格度**：`wow.discussLevel` 设置（`standard` | `strict`，默认 `standard`）控制 discuss 模式是否注入额外的回答质量提醒行。`standard` 保持探讨直接（更少的确认提示——适合强模型或简单项目）；`strict` 在信息缺失/模糊/高风险时提出精简问题。在 `/config:project` 或 `/config:global` 中设置，project 覆盖 global。仅影响 discuss 模式。
