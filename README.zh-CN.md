...
向 system prompt 添加基于 OS locale 的硬性 `[LANGUAGE]` 指令：该指令明确点名目标语言（如 `中文（简体）`），并要求技术标识符、代码、路径、命令和 commit messages 保持原语言。这比每轮推断用户语言更可靠——当输入同时包含自然语言和英文代码/路径/命令时，每轮推断容易误判。在单机上检测到的 OS locale 跨轮稳定，因此 prompt prefix cache 不受影响。`localeToDisplayName()` 会规范化 script 子标签（`zh-Hans-CN` → `zh-CN`、`zh-Hant` → `zh-TW`），从而准确区分简体/繁体中文。
...
通过独立 LLM 调用生成本地 git 分支名和 start point，创建前会先让你 review。可以传入任务描述；也可以省略描述，让 Wow 根据最近聊天上下文、`git status` 和当前 diff 摘要推断分支名与起点。

示例：

```txt
/git-branch 修复 OAuth 回调后的登录跳转问题
/git-branch --from main 添加项目配置界面
/git-branch 从 develop 切一个修复历史搜索的分支
```
...
- 创建前允许编辑 branch name
- 未提供 `--from` 时，执行过程中允许选择或手动输入 start point
- `--from` 补全支持本地分支和已有 remote-tracking branches；不会自动执行 `git fetch`
- remote-tracking branch 只作为创建起点使用，不会设置 upstream tracking
- 工作区有未提交改动时会先提示确认；当 start point 与当前 `HEAD` 不同时会额外提示风险
- 仅当生成的本地分支名已存在时才追加时间戳
...
注册的 tools：`codegraph_explore`、`codegraph_node`、`codegraph_search`、
`codegraph_callers` 和 `codegraph_status`。适用于结构化代码探索、符号关系、调用方和影响范围问题。
Tool 输出会在 LLM context 中限制为 32KB，超大完整输出保存到临时文件。CodeGraph tools 可在
human-led discuss/plan/revise 探索模式中使用。
...
- 移除 `wow-tui` 不应破坏 workflow、cache、commit、codegraph 或 webfetch 行为。
...
- **Prefix-cache safety**：不要向 system prompt 添加 per-turn timestamp/random ID；不要为 modes 切换 active tools；返回给 LLM 前截断 custom tool output
