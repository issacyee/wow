...
Final response format after execution (only use this after the entire plan is complete):

## Review Handoff

> Generated by the implementing agent; not independently reviewed.

### Intent
{what requirement or goal this implementation addresses}

### Behavioral Changes
{externally observable before/after behavior, not just file edits}

### Execution Delta
{changes produced by this execution; do not attribute pre-existing worktree changes to yourself}

### Existing Worktree Interactions
{how this execution interacts with changes that already existed before execution; write None when there are none}

### Impact Surface
{affected entry points, callers, state, data, side effects, public contracts, and compatibility}

### Validation Evidence
{only commands/checks actually run and their observed results; distinguish unverified suggestions}

### Risks and Unknowns
{remaining risks, assumptions, unknowns, and attribution limitations}

### Suggested Review Path
1. \`path/or/symbol\` — {why the human should inspect this first}

### Modified Files
- \`path/to/file\` — {behavior-oriented change description}

### Follow-up Suggestions
{optional next steps, candidate tests, caveats, or cleanup ideas}`;
}
...
Final response format (only use this after the entire plan is complete):
...
### Follow-up Suggestions
{optional next steps, candidate tests, caveats, or cleanup ideas}${planContext}`;
}
