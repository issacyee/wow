...
│   │   ├── locale.ts        # Locale detection and OS-locale language instruction utilities
...
│   │   ├── ask-panel.ts     # discuss :::ask structured-question overlay (consumes logic-layer ask trigger)
│   │   ├── instance-title.ts # Session/instance terminal title
│   │   ├── terminal-focus.ts # Terminal focus protocol owner for notifications
...
- **path-text.ts** — `shortenPlainPath()`. UI-independent path shortening for notifications and other plain-text channels.
...
- **Evidence-first aligned discuss flow**: when the request has investigable factual context, discuss mode must first perform proportional read-only research into relevant local code, configuration, documentation, Git state, and structural relationships; dependency documentation or external sources are used only when local evidence is insufficient and the external fact materially affects the discussion. If the investigation target cannot be located, it may ask exactly one minimal scoping question first. Before asking substantive, non-dependent question batches, it briefly presents the observed current state, the gap to the human's goal, and the remaining decision-relevant unknowns. It never asks for facts the allowed tools can discover, does not force questions when no such unknown remains, and re-investigates when an answer changes scope or invalidates an assumption. It follows up until it is at least 95% confident about the real needs and goals, then summarizes the shared understanding and proposes a direction without turning it into a plan unless the user asks with `??`.
