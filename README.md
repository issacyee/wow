...
- **Evidence-first aligned discussion**: when the request has investigable factual context, discuss mode first performs proportional read-only research into relevant local code, configuration, documentation, Git state, and structural relationships; it uses external sources only when local evidence is insufficient. Before asking decision-relevant, non-dependent questions, it briefly presents the observed current state, the gap to the user's goal, and the remaining unknowns. It does not ask for discoverable facts or force questions when none are needed, and it follows up until it is highly confident about the user's real needs and goals before proposing a direction without turning it into a plan unless `??` is used.
